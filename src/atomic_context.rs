        ...
        for mono_item in mono_items {
            if let MonoItem::Fn(instance) = mono_item {
                    ...
                    && ty.is_slice()
                {
                    ...
                    // length array drop which is easy to analyze to hard-to-analyze slice.
                    //
                    // Luckily these drop glues are locally monormphized so they shouldn't be called
                    // cross-crate. Omitting them here and check them through callers (in array's
                    // lowering case, `[T; N]`, which we can analyze directly without peek into MIR
                    // lowering).
                    continue;
                }
